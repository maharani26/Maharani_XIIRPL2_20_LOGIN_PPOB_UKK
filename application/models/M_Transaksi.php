...
class M_Transaksi extends CI_Model {
	...
	public function cekstok(){
		...
		for($i=0;$i<count($this->cart->contents());$i++){
			...
			$stok = $this->db->where('kode_sepeda', $this->input->post('kode_sepeda')[$i])->get('sepeda')->row()->stok;
	...
	public function simpan_db(){
			...
			$this->db->where('kode_sepeda', $this->input->post('kode_sepeda')[$i])->update('sepeda', $u_stok);
			}
			...
			$s_dttransaski[] = array('kode_transaksi' => $ts->kode_transaksi,
									 'kode_sepeda'=>$this->input->post('kode_sepeda')[$i],
		...
		public function ambil_dts($kode_transaksi){
			...
			return $this->db->join('sepeda', 'sepeda.kode_sepeda = detail_transaksi.kode_sepeda')
